...
def main():
    ...
    Q_values = np.zeros(world.observation_space() + (4,))
    episodes = 100000
    sarsa_n_values = 3
    discountfactor = 0.9
    learningRate = 0.1
    ...
    for i in range(episodes):
        ...
        while not done:
            ...
            if len(laststeps) >= sarsa_n_values - 1:
                ...
                newQVal = summedRewards + (discountfactor ** sarsa_n_values) * Q_values[state[1], state[0], action]
        ...
        while len(laststeps) > 0:
            summedRewards = 0
            for j in range(len(laststeps)):
                summedRewards += laststeps[j][2] * (discountfactor ** j)   
            oldQVal = Q_values[laststeps[0][0][1], laststeps[0][0][0], laststeps[0][1]]
            Q_values[laststeps[0][0][1], laststeps[0][0][0], laststeps[0][1]] = oldQVal * (1 - learningRate) + learningRate * summedRewards
            laststeps = laststeps[1:]
        
        if visualizeSteps==False and i%500 == 499:
...
def visualize_qvalues(qValues):
    ...
    for i in range(qValues.shape[1]*3):
        ...
        for j in range(qValues.shape[0]*3):
            ...
            if i%3==0:
                if j%3==1:
                    printRow += '%05.2f' % qValues[int(i/3)][int(j/3)][1]
                else:
                    printRow += ' '*4
            elif i%3==1:
                if j%3==0:
                    printRow += '%05.2f' % qValues[int(i/3)][int(j/3)][0]
                elif j%3==2:
                    printRow += '%05.2f' % qValues[int(i/3)][int(j/3)][2]
                else:
                    printRow += ' '*3
            elif i%3==2:
                if j%3==1:
                    printRow += '%05.2f' % qValues[int(i/3)][int(j/3)][3]
                else:
                    printRow += ' '*4
        print(printRow)
